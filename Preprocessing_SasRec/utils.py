...
class SeqDataset_Inference(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #if user_id not in self.user_valid or len(self.user_valid[user_id]) == 0:
        # Skip to next sample (wrap around safely)
...
def evaluate(model, dataset, args, top_k=1):
    ...
    for u in users:
        ...
        rated = set(train[u])
        rated.add(0)
        item_idx = [test[u][0]]  # The first item is the true item
        for _ in range(10):
        ...
        # Always detach first, then move to CPU, then convert to numpy
        ...
        # Calculate rank - find position of true item (which is at index 0 in item_idx)
        # Sort predictions in descending order (higher is better) and find where true item (index 0) is
        rank = np.where(np.argsort(predictions)[::-1] == 0)[0][0]

        valid_user += 1

        # For HR@k, check if rank is less than k
        if rank < top_k:
            HT += 1
            
        # For NDCG@k, calculate proper NDCG value if within top k
        if rank < top_k:
...
# evaluate on val set
def evaluate_valid(model, dataset, args,top_k=1):
    [train, valid, test, usernum, itemnum] = copy.deepcopy(dataset)
